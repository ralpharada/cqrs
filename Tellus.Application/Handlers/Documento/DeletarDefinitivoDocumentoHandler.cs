...
{
    public class DeletarDefinitivoDocumentoHandler : IRequestHandler<DeletarDefinitivoDocumentoPorIdQuery, IEvent>
    {
        ...
        public DeletarDefinitivoDocumentoHandler(IDocumentoRepository repository, UsuarioAutenticado usuarioAutenticado, IUsuarioRepository repositoryUsuario, IClienteRepository repositoryCliente, IAmazonRepository repositoryAmazon, IConfiguration configuration)
        {
        ...
        public async Task<IEvent> Handle(DeletarDefinitivoDocumentoPorIdQuery request, CancellationToken cancellationToken)
        {
            string mensagem = "";
            bool success = false;
            try
            {
                ...
                if (usuario != null)
                {
                    ...
                    if (cliente != null && cliente.Status)
                    {
                        ...
                        success = await _repository.DeletarPorIds(new List<Guid> { request.Id }, usuario.ClienteId, cancellationToken);
