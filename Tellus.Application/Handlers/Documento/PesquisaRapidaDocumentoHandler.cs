...
{
    public class PesquisaRapidaDocumentoHandler : IRequestHandler<PesquisaRapidaDocumentoQuery, IEvent>
    {
        ...
        public PesquisaRapidaDocumentoHandler(IDocumentoRepository repository,
        ...
        public async Task<IEvent> Handle(PesquisaRapidaDocumentoQuery request, CancellationToken cancellationToken)
        {
            string mensagem = "";
            bool success = false;
            Guid id = Guid.Empty;
            ...
            try
            {
                ...
                if (usuario != null)
                {
                    ...
                    if (cliente != null && cliente.Status)
                    {
                        ...
                        var result = await _repository.PesquisaRapida(usuario.ClienteId, request.TipoDocumentoId, request.Pesquisa, request.CurrentPage, Convert.ToInt32(_configuration.GetSection("PageSize").Value), cancellationToken);
                        total = await _repository.CountPesquisaRapida(usuario.ClienteId, request.TipoDocumentoId, request.Pesquisa, cancellationToken);
